...
const API_URL = "https://noticias-backend-24ee.onrender.com";
...
async function carregarNoticias() {
  ...
  try {
    ...
    mensagem.innerHTML = `<div class="error">❌ Erro ao conectar ao backend: ${erro.message}<br/><small>URL: ${API_URL}</small></div>`;
    container.innerHTML = '';
  }
}
...
async function criarNoticia() {
  ...
  try {
    const response = await fetch(`${API_URL}/noticias`, {
      ...
      body: JSON.stringify({ titulo, descricao, categoria: categoria || "Geral" })
    });
    if (!response.ok) throw new Error(`Erro HTTP: ${response.status}`);
    ...
    setTimeout(() => { carregarNoticias(); mensagem.innerHTML = ''; }, 1500);
  } catch (erro) {
